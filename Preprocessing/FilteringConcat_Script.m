...
%                 plot(concat_data.time, concat_data.chan1,'r');
                ...
                load(THIS_SESSION_BL);
                ca_data.chan1_filt = concat_data.chan1_filt(1:length(ca_data.time));
                ca_data.chan2_filt = concat_data.chan2_filt(1:length(ca_data.time));
                baseline_length = length(ca_data.time);
                save([ANALYZED_DATA INDICATOR_FOLDER thisMonth '/baseline_' animalInfo '_filtered.mat'], 'ca_data');
                %% save filtered data - CNO
                load(THIS_SESSION_CNO);
                ca_data.chan1_filt = concat_data.chan1_filt(baseline_length+1:end);
