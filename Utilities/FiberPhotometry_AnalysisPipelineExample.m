...
% Analyzed data path - where to store outputs of functions
ANALYZED_DATA= [BASEPATH 'Analyzed_Data\'];
% Figure path - where to store figures
...
ca_data.chan1_filt = filter_2sIIR(ca_data.chan1,cutoff_freq,samp_freq,IIR_order,filter_type)+mean(ca_data.chan1(1:100));
% filter data for channel 2
ca_data.chan2_filt = filter_2sIIR(ca_data.chan2,cutoff_freq,samp_freq,IIR_order,filter_type)+mean(ca_data.chan2(1:100));
%% Plot filtered Data
% for channel 1
subplot(2,1,1);
...
stdDev.chan1 = std(ca_data.chan1);
dataMean.chan1 = mean(ca_data.chan1);
stdDev.chan2 = std(ca_data.chan2);
dataMean.chan2 = mean(ca_data.chan2);
...
largeEvents.chan1_idx = find(concat_data.chan1 >= (dataMean.chan1+thr_large.chan1));
largeEvents.chan1_values = concat_data.chan1(largeEvents.chan1_idx);
largeEvents.chan2_idx = find(concat_data.chan2 >= (dataMean.chan2+thr_large.chan2));
largeEvents.chan2_values = concat_data.chan2(largeEvents.chan2_idx);
...
smallEvents.chan1_idx = find(concat_data.chan1 >= (dataMean.chan1+thr_small.chan1)...
    & concat_data.chan1 < (dataMean.chan1+thr_large.chan1));
smallEvents.chan1_values = concat_data.chan1(smallEvents.chan1_idx);
smallEvents.chan2_idx = find(concat_data.chan2 >= (dataMean.chan2+thr_small.chan1)...
    & concat_data.chan2 < (dataMean.chan2+thr_large.chan2));
smallEvents.chan2_values = concat_data.chan2(smallEvents.chan2_idx);
%% Plot calcium events for both channels, small vs large
figure;
%plot channel 1 small events
subplot(4,1,1);
plot(ca_data.time,ca_data.chan1,'b');
hold on;
...
subplot(4,1,3);
plot(ca_data.time,ca_data.chan2,'b');
hold on;
